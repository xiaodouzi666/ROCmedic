...
UPLOAD_FOLDER = '~/Xray/upload_images'
...
def get_diagnosis(image_path):
    command = f"python cli_xray_fit_flask.py --image_path {image_path} --english"
    ...
    for line in output_lines:
        if 'Number of text tokens' in line:
            capturing = True
            continue
        if capturing and not line.startswith('Welcome to XrayGLM model'):
...
def get_suggestion(diagnosis_result):
    ...
    suggestion = " ".join(inference_output).replace("Yes, ", "")
    return {
