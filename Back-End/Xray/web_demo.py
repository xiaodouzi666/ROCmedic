...
def custom_url_ok(url: str) -> bool:
    try:
        r = httpx.head(url, timeout=30, verify=False)
        return r.status_code == 200
    except httpx.RequestError:
        return False

gr.networking.url_ok = custom_url_ok
    ...
    with gr.Blocks(css='style.css') as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=5):
                result_text = gr.Chatbot(label='Multi-round conversation History', value=[("", "Hi, What do you want to know about this image?")], height=550)

        gr.Markdown(NOTES)

        print(gr.__version__)
        run_button.click(fn=request_model,inputs=[input_text, temperature, top_p, image_prompt, result_text],
                         outputs=[input_text, result_text], concurrency_limit=10)
        input_text.submit(fn=request_model,inputs=[input_text, temperature, top_p, image_prompt, result_text],
                         outputs=[input_text, result_text], concurrency_limit=10)
        clear_button.click(fn=clear_fn, inputs=clear_button, outputs=[input_text, result_text, image_prompt], concurrency_limit=10)
        image_prompt.upload(fn=clear_fn2, inputs=clear_button, outputs=[result_text], concurrency_limit=10)
        image_prompt.clear(fn=clear_fn2, inputs=clear_button, outputs=[result_text], concurrency_limit=10)

        print(gr.__version__)

    demo.launch(share=True, max_threads=10, server_port=7860, server_name="0.0.0.0", prevent_thread_lock=True)
