...
class FewShotDataset(Dataset):
    def __init__(self, path, processor, tokenizer, args):
        ...
        self.labels = []
        for item in data:
            image = processor(Image.open(item['image']).convert('RGB'))
            ...
            suggestion_text = " ".join(item.get('diagnosis_suggestions', []))
            suggestion_ids = tokenizer.encode(text=suggestion_text, add_special_tokens=False)
            ...
            if len(suggestion_ids) > args.max_target_length - 2:
                suggestion_ids = suggestion_ids[: args.max_target_length - 2]
            pre_image = len(input0)
            input_ids = tokenizer.build_inputs_with_special_tokens(a_ids, b_ids + suggestion_ids)
